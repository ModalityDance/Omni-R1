...
CHAMELEON_START_DOCSTRING = r"""self.image_seq_length
    This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the
    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
    etc.)
...
class ChameleonCFGProcessor:
    """
    Implements Classifier-Free Guidance for Chameleon models.

    This processor handles the logic for both text and image guidance,
    applying different scaling factors to control the generation.
    """

    def __init__(
        self, 
        guidance_scale: float = 5.0,
        guidance_scale_full: float = 5.0, 
        guidance_scale_image: float = 1.2,
        guidance_scale_negative: float = 3.0,
        guidance_scale_original_prompt: float = 5.0,
        config: ChameleonConfig = None
    ):
        """
        Initialize the CFG processor.

        Args:
            guidance_scale_full: Guidance scale for text conditioning
            guidance_scale_image: Guidance scale for image conditioning
            guidance_scale_negative: Guidance scale for negative conditioning
            guidance_scale_original_prompt: Guidance scale for the original prompt
        """
        self.guidance_scale_full = guidance_scale_full
        self.guidance_scale_image = guidance_scale_image
        self.guidance_scale_negative = guidance_scale_negative
        self.guidance_scale_original_prompt = guidance_scale_original_prompt
        self.config = config


    def process_logits(self, logits: torch.FloatTensor, cfg_config) -> torch.FloatTensor:
        """
        Apply CFG to the logits by combining conditional and unconditional outputs.

        Args:
            logits: Model output logits [3*batch_size, vocab_size]

        Returns:
            Processed logits with CFG applied [batch_size, vocab_size]
        """
            
        if cfg_config == "normal":
            batch_size = logits.shape[0] // 3

            # Split the logits into the three conditions
            full_conditioned_logits = logits[:batch_size]
            unconditioned_logits = logits[batch_size:2*batch_size]
            image_conditioned_logits = logits[2*batch_size:]

            mixed_logits = (
                unconditioned_logits
                + self.guidance_scale_full * (full_conditioned_logits - unconditioned_logits)
            ...
            full_conditioned_logits = logits[:batch_size]
            unconditioned_logits = logits[batch_size:2*batch_size]
            negative_logits = logits[2*batch_size:]

            mixed_logits = (
                unconditioned_logits
                + self.guidance_scale_full * (full_conditioned_logits - unconditioned_logits)
                - self.guidance_scale_negative * (negative_logits - unconditioned_logits)
            )
        elif cfg_config == "full":
            batch_size = logits.shape[0] // 5

            # Split the logits into the three conditions
            full_conditioned_logits = logits[:batch_size]
            image_conditioned_logits = logits[batch_size:2*batch_size]
            unconditioned_logits = logits[2*batch_size:3*batch_size]
            negative_logits = logits[3*batch_size:4*batch_size]
            original_prompt_logits = logits[4*batch_size:]

            mixed_logits = (
                unconditioned_logits
                + self.guidance_scale_full * (full_conditioned_logits - image_conditioned_logits)
                + self.guidance_scale_image * (image_conditioned_logits - unconditioned_logits)
                - self.guidance_scale_negative * (negative_logits - unconditioned_logits)
                + self.guidance_scale_original_prompt * (original_prompt_logits - unconditioned_logits)
            )

        return mixed_logits


class ChameleonForConditionalGenerationWithCFG(ChameleonForConditionalGeneration):
    """
    ChameleonForCausalLM with integrated Classifier-Free Guidance
    """

    def __init__(self, config):
        super().__init__(config)
        self.cfg_processor = None
    ...
    def setup_cfg(
        ...
        """
        Initialize the CFG processor with guidance scales.

        Args:
            guidance_scale_full: Guidance scale for full conditioning
            guidance_scale_image: Guidance scale for image conditioning
            guidance_scale_negative: Guidance scale for negative conditioning
            guidance_scale_original_prompt: Guidance scale for the original prompt
            image_tokens: List of token IDs that are part of the image vocabulary
            special_tokens: List of special token IDs
        """
        self.cfg_processor = ChameleonCFGProcessor(
            guidance_scale_full=guidance_scale_full,
            guidance_scale_image=guidance_scale_image,
            guidance_scale_negative=guidance_scale_negative,
            guidance_scale_original_prompt=guidance_scale_original_prompt,
            config=config
        )
    ...
    def forward(
        ...
        """
        Forward pass with CFG processing if enabled.
        """
        ...
        # Apply CFG to logits if enabled and we're in generation mode (no labels)
        if self.cfg_config != "no" and labels is None:
            processed_logits = self.cfg_processor.process_logits(outputs.logits, self.cfg_config)

            # Create new outputs with processed logits
            if return_dict:
                outputs.logits = processed_logits
            else:
                # If not returning dict, the logits are the first element
                outputs = (processed_logits,) + outputs[1:]
